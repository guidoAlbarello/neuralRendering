...
function Terrain(props) {
    ...
    useFrame(({ clock }) => {
        ...
        mesh.current.material.uniforms.movement.value = -movement.forward;
    });
    ...
    return (
        <mesh
            onPointerDown={(e) => {mouse.x = state.mouse.x; state.y=state.mouse.y;}}
            {...props}
            ref={mesh}
  ...
  */
  ...
  
  {
    ...
    mat3 rotationY = mat3(
        ...
        vec3(cos(6.283185*iMouse.x), 0.0, -sin(6.283185*iMouse.x)),

        vec3(0.0, 1.0, 0.0),

        vec3(sin(6.283185*iMouse.x), 0.0, cos(6.283185*iMouse.x))

        );
    mat3 rotationX = mat3(
        vec3(1.0, 0.0, 0.0),

            vec3( 0.0,cos(6.283185*iMouse.y), -sin(6.283185*iMouse.y)),
    
    
            vec3(0.0, sin(6.283185*iMouse.y), cos(6.283185*iMouse.y))
    
            );
    vec3 dir = normalize(rotationY*rotationX*rayDirection(45.0, iResolution.xy, fragCoord));
  
    vec3 camera =rotationY*rotationX *vec3(0.0,0.0,400.0+movement);
